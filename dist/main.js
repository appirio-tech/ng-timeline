(function() {
  ...
  dependencies = ['ui.router', 'ngResource', 'app.constants', 'appirio-tech-messaging', 'appirio-tech-ng-auth'];
...
(function() {
  ...
  TimelineController = function(TimelineService, $stateParams, UserV3Service, ThreadsAPIService, CoPilotAPIService, AuthService, $scope) {
    ...
    getOrCreateThread = function() {
      return $scope.$watch(UserV3Service.getCurrentUser, function() {
        var params, publishers, resource, thread, user;
        user = UserV3Service.getCurrentUser();
        if (user) {
          vm.subscriberId = user.id;
        }
        publishers = [vm.coPilotId, vm.subscriberId];
        params = {
        ...
        return resource.then(function(response) {
          ...
          return TimelineService.getUnreadCount(vm.threadId, vm.subscriberId, setUnreadCount);
        });
      });
    };
  ...
  TimelineController.$inject = ['TimelineService', '$stateParams', 'UserV3Service', 'ThreadsAPIService', 'CoPilotAPIService', 'AuthService', '$scope'];
